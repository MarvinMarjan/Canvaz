...
public static class FloatArrayConverterExtensions
{
    ...
    public static Vec2f ToVec2f(this float[] values)
        => new(values[0], values[1]);

    public static Vec2i ToVec2i(this float[] values)
        => new((int)values[0], (int)values[1]);

    public static Vec2u ToVec2u(this float[] values)
        => new((uint)values[0], (uint)values[1]);

    
    public static ColorRGBA ToColorRGBA(this float[] values)
        => new((byte)Math.Clamp(values[0], 0, 255),
            (byte)Math.Clamp(values[1], 0, 255),
            (byte)Math.Clamp(values[2], 0, 255),
